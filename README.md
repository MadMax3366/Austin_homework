...
| 运营 | 名下 100／全机构 1,000 名学生、试听与低课时队列、全局课表、排课资源 | 搜索分页、建咨询、排试听、记录结论、转报名、完成跟进、为本人学生代建续费订单 |
...
`demo:setup` 会依次构建、把所有未应用迁移写入同一个 `.wrangler/state` 本地 D1，并执行可重复 Seed。基线包含 1,000 名学生、10 位运营（每人 100 名）、20 位老师和 60 个每周固定班；再次执行不会重复业务数据。

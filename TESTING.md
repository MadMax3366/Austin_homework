...
- 运营：1,000 人目录的总数、参数化搜索、分页和通配符转义；owner-scoped 试听／低课时队列、咨询、转正式、续费与跨 owner 写入拦截。
...
最终一次 clean-state 运行通过 75 个 HTTP 场景（含预期的 400／403／404／409／422），并在所有写操作后再次加载每个工作台。
